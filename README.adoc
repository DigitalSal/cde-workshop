...
* Alternatively, if you don't have `git` installed on your machine, create a folder on your local computer; navigate to https://github.com/DigitalSal/cde-workshop.git[this URL] and manually download the files.
...
Create two CDE Spark Jobs using scripts "<cdeuser>-05-A-ETL.py" and "<cdeuser>-05-B-Reports.py" but do not run them.
...
### Bonus: Using the Spark Migration Tool
...
### Bonus Lab: Using CDE Airflow with CDW
...
Next, create a new Airflow CDE Job named "<cdeuserXX>-CDW-Dag". Upload the new DAG file to the same or a new CDE resource as part of the creation process.
...
### Bonus Lab: Using the CDE Airflow Editor to Build Airflow DAGs without Coding
